...
int bitcoin_init(void) {
  ...
  global_ctx = secp256k1_context_create(SECP256K1_CONTEXT_SIGN |
...
int derive_pubkey(const unsigned char *privkey,
  ...
  if (global_ctx == NULL) {
    fprintf(
        stderr,
        "Error: Bitcoin library not initialized. Call bitcoin_init() first.\n");
    return 0;
  }
  ...
  secp256k1_ec_pubkey_serialize(global_ctx, pubkey_compressed, &output_len,
